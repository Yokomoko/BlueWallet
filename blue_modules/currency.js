...
function satoshiToLocalCurrency(satoshi, format = true) {
  ...
  let b = new BigNumber(satoshi).dividedBy(100000000).multipliedBy(exchangeRates['GRS_' + preferredFiatCurrency.endPointKey]);
...
async function mostRecentFetchedRate() {
  ...
  return {
    ...
    Rate: formatter.format(currencyInformation[`GRS_${preferredFiatCurrency.endPointKey}`]),
  };
}
...
function fiatToBTC(fiatFloat) {
  ...
  b = b.dividedBy(exchangeRates['GRS_' + preferredFiatCurrency.endPointKey]).toFixed(8);
  return b;
}
 ...
 * Used to mock data in tests
 *
 * @param {string} pair as expected by rest of this module, e.g 'GRS_JPY' or 'GRS_USD'
