  ...
  processAddressData = data => {
    this.setState({ isLoading: true }, async () => {
      ...
      if (this.state.fromWallet.isAddressValid(dataWithoutSchema)) {
        ...
        console.log(options);
        if (btcAddressRx.test(address) || address.indexOf('grs1') === 0 || address.indexOf('GRS1') === 0) {
          const units = this.state.units;
              ...
              >
                ...
                <View style={styles.feeRow}>
                  <Text style={styles.feeValue}>
                    {this.state.feePrecalc.current ? this.formatFee(this.state.feePrecalc.current) : this.state.fee + ' gro/byte'}
                  </Text>
                </View>
              </TouchableOpacity>
