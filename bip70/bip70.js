...
export default class BitcoinBIP70TransactionDecode {
  static decode(data) {
    return new Promise(async (resolve, reject) => {
      try {
        let url;
        if (data.match(/groestlcoin:\?r=https?:\/\/\S+/gi)) {
          url = data.toString().split('groestlcoin:?r=')[1];
  ...
  static matchesPaymentURL(data) {
    return data !== null && (data.match(/groestlcoin:\?r=https?:\/\/\S+/gi) !== null || data.startsWith('https://bitpay.com/i/') || data.startsWith('https://www.bitpay.com/i/'));
  }
}
