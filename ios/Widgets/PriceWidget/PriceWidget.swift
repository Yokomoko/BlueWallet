...
struct PriceWidgetProvider: TimelineProvider {
  ...
  func placeholder(in context: Context) -> PriceWidgetEntry {
    return PriceWidgetEntry(date: Date(), currentMarketData: MarketData(nextBlock: "", sats: "", price: "$1", rate: 1, dateString: "2019-09-18T17:27:00+00:00"))
  }

  func getSnapshot(in context: Context, completion: @escaping (PriceWidgetEntry) -> ()) {
    let entry: PriceWidgetEntry
    if (context.isPreview) {
      entry = PriceWidgetEntry(date: Date(), currentMarketData: MarketData(nextBlock: "", sats: "", price: "$1", rate: 1, dateString: "2019-09-18T17:27:00+00:00"))
    } else {
  ...
  func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
    ...
    if (context.isPreview) {
      let entry = PriceWidgetEntry(date: Date(), currentMarketData: MarketData(nextBlock: "", sats: "", price: "$1", rate: 1, dateString: "2019-09-18T17:27:00+00:00"))
...
struct PriceWidget_Previews: PreviewProvider {
  static var previews: some View {
    PriceWidgetEntryView(entry: PriceWidgetEntry(date: Date(), currentMarketData: MarketData(nextBlock: "", sats: "", price: "$1", rate: 1, dateString: "2019-09-18T17:27:00+00:00")))
