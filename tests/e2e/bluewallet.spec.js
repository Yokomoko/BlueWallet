...
describe('BlueWallet UI Tests', () => {
  ...
  it('can import BIP84 mnemonic, fetch balance & transactions, then create a transaction; then cosign', async () => {
    ...
    await helperImportWallet(process.env.HD_MNEMONIC_BIP84, 'Imported HD SegWit (BIP84 Bech32 Native)', '0.00105526 GRS');
    ...
    const bip21 = 'groestlcoin:grs1q6ve7qrz0gg9tt22022rx620uepkpkk2ed286gw?amount=0.0001&pj=https://grspay.com/GRS/pj';
    ...
    assert.strictEqual(bitcoin.address.fromOutputScript(transaction.outs[0].script), 'grs1q6ve7qrz0gg9tt22022rx620uepkpkk2ed286gw');
  ...
  it.skip('can import zpub as watch-only, import psbt, and then scan signed psbt', async () => {
    ...
    await helperImportWallet(
      ...
      'Imported Watch-only',
      '0 GRS', // it used to be 0.00030666 till someone stole it from git history kek
    );
  ...
  it('should handle URL successfully', async () => {
    ...
    await device.launchApp({
      newInstance: true,
      url: 'groestlcoin:GRS1Q6VE7QRZ0GG9TT22022RX620UEPKPKK2ED286GW\\?amount=0.0001\\&label=Yo',
    });
  ...
  it.skip('can import multisig setup from UR (ver1) QRs (2 frames), and create tx', async () => {
    const lockFile = '/tmp/travislock.' + hashIt(jasmine.currentTest.fullName);
    if (process.env.TRAVIS) {
      if (require('fs').existsSync(lockFile))
        return console.warn('skipping', JSON.stringify(jasmine.currentTest.fullName), 'as it previously passed on Travis');
